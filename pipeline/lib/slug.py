...
def file_id(pid):
    """slug 为空时的文件名兜底:把 DOI(sources.id)清洗成安全的文件名片段。
...
def unique_slug(conn, title, paper_id):
    """Ensure the slug is unique within sources.slug (excluding the same id)."""
    base = slugify(title)
    cand, n = base, 1
    while conn.execute("SELECT id FROM sources WHERE slug=? AND id<>?", (cand, paper_id)).fetchone():
        n += 1
        cand = f"{base}_{n}"
    return cand
