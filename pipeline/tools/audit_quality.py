"""Retroactive quality audit of sources already committed to a topic.
...
Default is dry-run (report only); --apply removes block-tier sources from the
topic (and from sources/citations if no other topic references them) and
...
def remove_papers(conn, topic_id, paper_ids):
    """Remove sources from a topic; delete orphans (no other topic) + their citation edges."""
    removed_files = []
    for pid in paper_ids:
        conn.execute("DELETE FROM source_topic WHERE topic_id=? AND paper_id=?", (topic_id, pid))
        other = conn.execute("SELECT COUNT(*) c FROM source_topic WHERE paper_id=?", (pid,)).fetchone()["c"]
        if other == 0:
            row = conn.execute("SELECT source_path FROM sources WHERE id=?", (pid,)).fetchone()
            if row and row["source_path"]:
                removed_files.append(row["source_path"])
            ...
            conn.execute("DELETE FROM sources WHERE id=?", (pid,))
    rows = conn.execute(
        """SELECT pt.paper_id FROM source_topic pt JOIN sources p ON p.id=pt.paper_id
...
def main():
    ...
    # verdict 回写 sources 表(quality_tier/quality_signals),下游(总结/渲染/RAG)按标记行事
    for r, q in verdicts:
        conn.execute("UPDATE sources SET quality_tier=?, quality_signals=? WHERE id=?",
    ...
    lines = [f"# Quality audit — {topic_id}", f"_generated: {now_iso()}  sources: {len(rows)}  "
    ...
    if apply_mode and blocked:
        ...
        conn.commit()
        log.info(f"  APPLIED: removed {len(blocked)} sources, rank recomputed")
        if files:
