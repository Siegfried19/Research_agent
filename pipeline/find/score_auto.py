...
def prompt(idea, batch, anchors=None, hit_criteria=None):
    lst = "\n".join(
        f"- id: {c['id']}\n  title: {c['title']}\n  venue: {c.get('venue') or '(未知)'}\n"
        f"  abstract: {(c.get('abstract') or '(无摘要)')[:1200]}"
        for c in batch)
    hc = (f"\n**这一组属于一个子方向,它的「命中标准」(按它判相关,明显不符的直接压低分):**\n{hit_criteria}\n"
          if hit_criteria else "")
...
def boundary_rerank(score_dir, idea, anchors, allc, topic_id, band=8, k=5,
                    hit_criteria=None, file_glob="batch_*.json", centers=None, out_name="zz_boundary.json"):
    """只对"去留决策线"±band 分的窄带复称,治"漂移翻转去留"。把这十几篇塞进同一次调用
    (单一参照系)× k 次采样取均值(磨平生成抖动)。结果写 zz_*.json——文件名排在 batch_*.json
    之后,commit 的 sorted-glob 合并时按 id 覆盖,故无需改 commit.py。

    去留线 centers 默认=资格闸 {30, flag_min}(commit 的入库硬线;judgment 时代不再用全局 target)。
    已入库论文去留已定,不复称。file_glob/out_name 让每个 facet 用独立批文件 + 独立 zz 文件。"""
    merged = {}
    for f in sorted(score_dir.glob(file_glob)):
    ...
    if len(band_batch) < 2:
        log.info(f"  boundary_rerank SKIP: 边界带仅 {len(band_batch)} 篇(centers={centers})")
        return
    log.info(f"  boundary_rerank: centers={centers} band=±{band} → {len(band_batch)} 篇复称 × {k} 次取均值")
    acc = {i: [] for i in band_ids}
    for _ in range(k):
        try:
            for s in parse_scores(run_claude(prompt(idea, band_batch, anchors, hit_criteria))):
                if s["id"] in acc:
    ...
    if out:
        (score_dir / out_name).write_text(
            json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
        log.info(f"  boundary_rerank: {len(out)} 篇复称值写入 {out_name}(commit 合并时覆盖)")


def _scores_from(score_dir, glob_pat):
    scores = {}
    for f in sorted(score_dir.glob(glob_pat)):
        for s in json.loads(f.read_text(encoding="utf-8")):
            scores[s["id"]] = s
    return scores


def _pick_anchors(scores, cand_by_id, flag_min):
    """从一遍打分的分布里挑 3 张样卷(高=最高分/边界=最接近资格闸/低=最低分)。不写盘。"""
    if len(scores) < 3:
        return []
    ...
    for taught, s in [(95, ranked[0]), (flag_min, boundary), (10, ranked[-1])]:
        c = cand_by_id.get(s["id"]) or {}
        picks.append({"title": c.get("title") or s["id"], "score": taught, "reason": s.get("reason") or ""})
    return picks


def run_pass(candidates, idea, pass_anchors, score_dir, batch_size, concurrency, panel_on,
             tag, hit_criteria=None, file_prefix="", clear=True):
    """一遍打分 over `candidates` → 写 scores/{prefix}batch_<start>.json。返回 (scored, fail)。
    clear=True 先清空 scores/(单尺路径用);faceted 路径循环外清一次、循环内 clear=False。"""
    if clear:
        for f in score_dir.glob("*.json"):
            f.unlink()
    batches = [(s, candidates[s:s + batch_size]) for s in range(0, len(candidates), batch_size)]
    log.info(f"score_auto[{tag}]: {len(candidates)} candidates in {len(batches)} batches (size {batch_size}), "
    ...
    def worker(item, _i):
        ...
        arr = parse_scores(run_claude(prompt(idea, b, pass_anchors, hit_criteria)))
        ...
        (score_dir / f"{file_prefix}batch_{start}.json").write_text(
            json.dumps(clean, ensure_ascii=False, indent=2), encoding="utf-8")
        log.info(f"  OK batch {file_prefix}{start} ({len(clean)} scored)")
        return len(clean)
...
def main():
    ...
    # 统一走 facet 路:load_topic 把无 facets 的老主题归一成 1 个隐式 facet '_all'
    # (anchors←score_anchors, hit_criteria←preferences),这一条路覆盖所有主题,无新旧并行。
    tj = topic_dir / "topic.json"
    facets = T.load_topic(tj)["facets"] if tj.exists() else [
        {"key": "_all", "title": topic_id, "hit_criteria": "", "queries": [],
         "anchors": [], "seed_ids": [], "note": ""}]

    for f in score_dir.glob("*.json"):  # 整体清一次,之后各 facet 用独立前缀文件
        f.unlink()
    total_scored = total_fail = 0
    for fac in facets:
        key = fac["key"]
        group = [c for c in allc if (c.get("facet") or "_all") == key]
        if not group:
            log.info(f"score_auto facet '{key}': 0 candidates, skip")
            continue
        fac_anchors, hc, prefix = fac["anchors"], fac["hit_criteria"], f"{key}_"
        if not fac_anchors:  # 该 facet 无 anchors → in-memory 自举(不改 topic.json 意图档)
            log.info(f"score_auto facet '{key}': 无 anchors → 自举裸跑")
            run_pass(group, idea, [], score_dir, batch_size, concurrency, panel_on,
                     f"{key}/bare", hit_criteria=hc, file_prefix=prefix, clear=False)
            fac_anchors = _pick_anchors(_scores_from(score_dir, f"{prefix}batch_*.json"),
                                        {c["id"]: c for c in group}, flag_min)
            log.info(f"score_auto facet '{key}': 自举锚点 {[a['score'] for a in fac_anchors]}")
        sc, fl = run_pass(group, idea, fac_anchors, score_dir, batch_size, concurrency, panel_on,
                          f"{key}/anchored", hit_criteria=hc, file_prefix=prefix, clear=False)
        total_scored += sc
        total_fail += fl
        boundary_rerank(score_dir, idea, fac_anchors, group, topic_id,
                        hit_criteria=hc, file_glob=f"{prefix}batch_*.json",
                        out_name=f"zz_{key}_boundary.json")
    n_facets = len(facets)
    tag = f"faceted x{n_facets}" if n_facets > 1 else "single(_all)"
    log.info(f"score_auto done [{tag}]: {total_scored} scored, {total_fail} failed")
    run_log(topic_id, f"score_auto[{tag}]: {total_scored} scored, {total_fail} failed, facets={n_facets}")
