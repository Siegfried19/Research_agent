...
⛔ 收敛(别无限联想):候选池被代码硬顶在 4×target(discover 合并到顶会自动砍非种子尾部)。别靠无限扩检索词/翻引用/开子 agent 把池子越滚越大——那只拖慢打分还撞超时。每个 facet 覆盖够了就收手,宁可下轮再补,也别这一轮无止境地发散。
...
- 撞限流/网络/瞬时报错:你自己判断该重跑这步还是跳过继续,别因为单次报错就整轮停下;并发别开太猛刷崩学校访问。
