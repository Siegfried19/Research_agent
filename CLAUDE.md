...
> **记忆分布地图 + 换机器完整清单 → `change-device/MIGRATION.md`**（2026-06-10 建，回答"记忆在哪/换机器怎么办"；换机器流程另见 `change-device/README.md`）。
...
6. **🌟 大计划:把论文库变成"遇到问题就来查的知识库"(RAG over corpus)** —— 用户在别的 project 卡住时,让 agent 来这个库里检索答案。结论:**可行**(curated 语料+总结+PDF+引用图,RAG 底子好);**数据库大小不是瓶颈**(sqlite 只存元数据/摘要,原文在 store/pdfs 的 PDF;真正难点是"检索准不准")。
   ...
   - **★ 状态升级:②③ 阶段已落地(2026-06-18,详见 `logs/SESSION-2026-06-18-kb-retrieval.md`)** —— `ask.py` 现在 = **混合召回(FTS5 关键词 + Qwen3-Embedding-0.6B 向量,RRF 融合) → claude -p RCS 精挑 → 闭集引用/会说不知道**。内脏在新段文件夹 `pipeline/retrieve/{index,search,rerank,answer}.py`,嵌入引擎 `lib/embed.py`(非LLM,只把文字变坐标),旁路 `tools/similar.py`(找相似/揪重复,已抓出 2103.14295 重复入库)+ `tools/eval_retrieval.py`(A/B验收)。坐标库 `db/vec.sqlite`(gitignored,可重建,不碰生产库)。`ask.py` 留根、瘦身成总指挥;公共参数不变,加 `--no-rerank`。
     - ⚠️ **运行环境(2026-06-18 起自动纠偏,要紧)**:整条流水线统一跑在 conda 环境 **`research-agent`**(GPU torch/sentence-transformers/sqlite-vec;`environment.yml` 一键重建,是超集——主链只要 requests 也在内;模型缓存 `pipeline/retrieve/models/` 由 `lib/embed.py` 经 HF_HOME `setdefault` **自动钉好,无需手动前缀**)。**入口自动纠偏**:`run.py`/`ask.py` 顶部调 `lib/envguard.ensure_env()`——当前不在 research-agent 就用该环境 python **自动 re-exec 自己**,所以 `bash run.sh`/`python3`/cron/bot 哪种方式起都自动落到对的环境(主链子进程经 `sys.executable` 全继承;按环境名探测不写死路径,cron 干净环境靠 `~/anaconda3/envs/...` 兜底;逃生口 `RESEARCH_NO_REEXEC=1`)。找不到环境不拦着——主链在 base 也能跑,ask.py 自动回退纯 FTS。跑法直接 `python3 pipeline/ask.py "..." --answer` 即可(不再需要 `HF_HOME=... conda run` 前缀)。
     - 实测:GPU 215ms/篇(CPU 慢~100×,必须 GPU);种子 gold 纯FTS Recall@10 0.67→混合 1.00;cannot-answer 哨兵生效。**未做:Self-RAG式逐句自检(ISSUP)、真30-50条gold、合成层(⑤两段式/④引用图)。本机仅39篇有总结(库221篇),用户拟重做总结→重做后重跑 `index` 即增量跟上。**
