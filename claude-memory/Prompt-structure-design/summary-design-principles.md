...
## 四、由此推出的设计取向(已 deep-research 验证 + 落地，见 §八)
...
## 六、决策(用户 2026-06-17，中间态——次日 §八 定稿，留作过程记录)
...
机制改完后,进一步定了**总结生成 + 核查 prompt 本身怎么改**(上面 §三/§四A-B 的取向落到具体 prompt 文字)。核心:**去掉 note_plan + 接地门、总结回到"边读边写"**(note_plan 实测制造"无锚论断"假阳性洪水→版本通胀,且接地门只验引文在不在、防不住张冠李戴——危害>收益);数字让位 PDF;论断原子化+内联 strength;加"适用边界"段;codex 端告知数字立场防误报+加方向反转检查。**决策记录 + 去 note_plan 的证据 → `summary-prompt-rewrite-plan.md`；现行 prompt 原文以代码 + `prompts.md` 为准。** ✅ 已落地。
