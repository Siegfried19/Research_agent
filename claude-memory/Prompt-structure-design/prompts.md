...
| 06-16 | note_plan + 接地门两阶段(先列计划/逐句接地) | `claude-memory/modules-modification/summarize/STATE.md` |
| **06-18** | **去 note_plan/接地门 → 回边读边写**;数字让位 PDF;原子句+内联 strength;加"适用边界"段 | note_plan 实测危害>收益;`claude-memory/Prompt-structure-design/summary-design-principles.md §八` + `claude-memory/modules-modification/summarize/STATE.md` |

### 核查 prompt
| 日期 | 变化 | 缘由 / 出处 |
|---|---|---|
| 06-10 | Codex 核查 + `correct_summaries` 打补丁修正 | 跨模型评审团上线 |
| 06-15 | PDF 为唯一原文源;codex 自渲染看公式/图表 | commit 0d0ce64 / 26c3bb8;`claude-memory/modules-modification/verify/STATE.md` |
| 06-17 | 发现旧 correct 会**反向裁决核查员 + 伪造"已核对"背书** | `claude-memory/modules-modification/verify/STATE.md` |
| **06-18** | **report-only + major→整篇 resummarize**(删 correct_summaries);severity 加 `unverifiable` 态;**数字精度从宽**(claim 级语义核查,中等 effort) | 根治反向裁决 bug;`claude-memory/Prompt-structure-design/summary-design-principles.md §八` |

### 打分 prompt
| 日期 | 变化 | 缘由 / 出处 |
|---|---|---|
| 早期 | rubric 写死 digital-human 例子;batch 10 | — |
| **06-17** | **通用分档骨架 + `score_anchors` 注入 + 证据接地**;batch 10→20 + 批内洗牌;`boundary_rerank` 复称;无锚自举 | 治跨批"rubric execution drift";`claude-memory/Prompt-structure-design/score-drift-research-findings.md` + `claude-memory/modules-modification/find/STATE.md` |
