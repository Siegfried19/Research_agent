...
## 2026-06-21 17:46 EDT · ★ 出口重构：从"问答引擎"翻成"库地图 + 消费者拥有调查循环"

> 与用户从**需求**重谈，把出口设计推倒重来（来龙去脉见本条 + 全局账 `../../../claude_log.md` 同日条）。
> 这是本模块的**新主干**，上面那些"四模式/金字塔/全读"是攒出来的实现史，现降级。

**需求侧拍板（推导出新设计的几步）**
1. 出口①(人查) 与 ②(别项目 agent 查) **其实是同一回事**——人也是开个 agent 进库调查；消费者**永远是一个带上下文的 agent**。两套出口契约合一。
2. **消费者拥有调查循环**（用户定）：来查的 agent 自己驱动调查，我们**不该写死问答管道替它决定怎么查**（合"让 agent 自己判断、别写死它该判断的逻辑"纲领）。它"具体怎么干"由用户在**记忆**里给它写指令，不进我们代码。
3. 纪律强调：**质量优先、别图省 token、别过度依赖工具**（该深读就深读）。
4. **接入 = 项目 SSH 挂载到主力机、主力机 agent 访问** → 看到的就是**本地文件**。直接砍掉 `--json` 网络契约 / 远程 wrapper —— 全是本地文件读。
5. 运维现实：现在**只手动跑 summarize+verify**，不跑全自动链 → 地图新鲜度**不挂任何 pipeline 阶段**，改成"**agent 访问前现拼**"。

**落地（已写代码、已实测，未提交）**
- 🆕 `retrieve/map.py` —— **库地图生成器**（新主入口）。纯 stdlib（sqlite3+json+datetime），**不走 envguard/不碰 conda**，挂载点裸 `python3` 可跑；输出 stdout + 写 `data-base/INDEX.md`（gitignored，与 fts/vec 同类派生物）。
  - 3 数据源 join/merge：`papers.sqlite`（标题/年/venue/质量/kind/原件路径/id + 最新总结版本）+ `topics/*/verify_status.json`（核查态）+ `topics/*/selected.json`（facet，不在 DB）。
  - 两级分组 **topic → facet**（老主题无 facet 退化单桶）；组内**按年份降序**（相关度是"对主题"的不是"对 agent 题目"的，**去掉**）。
  - 收录"**有料可读**"（总结文件 **或** 原件文件**真实存在**）；纯元数据不进。实测 319 篇/3 主题，137 有总结 / 190 仅 PDF。
  - 标记**说大白话 + 头部图例**（agent 不用背内部代码）：质量→`预印本(未评审)`/`掠夺刊嫌疑`/`〔网络源〕`（ok/trusted 不出）；核查→`重大存疑`(major)/`小瑕疵`(minor)/`未充分核查`(stale+unverified+unverifiable)，pass/无总结**不出**（minor 单列、不混进"未充分核查",2026-06-21 用户要求提示）。无 `[n]` 编号、无摘要、引用句柄用 `id`。
  - **体量**：319 篇≈86k 字符≈~39k token（已做路径去重:家目录只给一次,省 ~13%）。现给 agent **整张**地图。
  - ⚠️ **未决:体量天花板**（用户提"会不会太长 agent 看不过来"）：线性增长、几千篇会撞上下文上限。曾试过 `--index`/`--topic` 切片 + "grep 文件不整吞" 的姿势,**用户不喜欢、已撤回**,决定**先给整张、用户思考更好的办法**。先别再自作主张加切片。
- 🆕 `lib/verify_status.py` —— 把 `answer.py` 的 `load_verify_status`/`resolve_verify` 抽成 stdlib 共享小函数（单一真相源，map 与 answer 共用）；`answer.py` 改为 import。
- 🆕 `instruction-for-other-agent.md`（**项目根**,对外前门,2026-06-21 从 retrieve/ 移到根=外部 agent 挂载上来从根找"怎么用我"更显眼）—— **库侧调查指南**（目录布局/标记含义/先跑地图/引用纪律）；map 头部有指针指向它。**单一真相源,随挂载点走;消费端只在自己 CLAUDE.md 落一条"库在哪+先读这份"的短指针,纪律每次现读不抄存(2026-06-21 用户定)。** 调查纪律 **v1 已起草**(7 条:召回优先别预筛/质量优先别省 token/总结主力+精确论断下钻原件/认标记/闭集引用不编造/撑不起就老实说/替别人查交付要能用),用户可再润色。
- ♻️ `ask.py` 顶部加**降级说明**：问答引擎（理解→召回→精排→生成答案）**降级为大库备用工具**，留盘不删、不再是默认；主入口改 `map.py`。readall/understand/search/rerank/index 同此定位。

**未决 / 下一步**
- 各消费端 CLAUDE.md 落"库在哪 + 先读 instruction-for-other-agent.md"短指针（待真用时加；全局 `~/.claude/CLAUDE.md` 的旧出口②指针 2026-06-16 撤过，现 map.py 免费 stdlib，可加回但指向 map/instruction、别指 ask.py）。
- 大库切换/体量天花板见上"未决:体量天花板"条（用户思考中,先给整张）。`ask.py` 向量管道留盘当大库兜底。
- 代码未提交（等用户 push）。
